...
class BertForDST(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # conbine weight between current and previous cls
        self.hidden_layer = nn.Linear(config.hidden_size * 2, config.hidden_size)
    ...
    def forward(self,
        ...
        # TODO : Originally, we return original CLS to the next turn, we can try to to return modified CLS insted, but it gonna be harder at interence time
        # feay1234
        # exploit CLS vector from previous turn
        if prev_cls is not None:
            # pooled_output = pooled_output * prev_cls
            # print(pooled_output.shape, prev_cls.shape)
            pooled_output = self.hidden_layer(torch.cat([pooled_output, prev_cls], dim=-1))
            # pooled_output = torch.cat([pooled_output, prev_cls], dim=-1)
        ...
        for slot in self.slot_list:
            ...
            if class_label_id is not None and start_pos is not None and end_pos is not None and refer_id is not None:
                ...
                if self.refer_index > -1:
                    per_example_loss = (self.class_loss_ratio) * class_loss * turn_weight + ((1 - self.class_loss_ratio) / 2) * token_loss * turn_weight + ((1 - self.class_loss_ratio) / 2) * refer_loss * turn_weight
                else:
                    per_example_loss = self.class_loss_ratio * class_loss * turn_weight + (1 - self.class_loss_ratio) * token_loss * turn_weight

                # print(per_example_loss.shape, turn_weight.shape)

                total_loss += per_example_loss.sum()
                per_slot_per_example_loss[slot] = per_example_loss

        # add hidden states and attention if they are here
        outputs = (total_loss,) + (per_slot_per_example_loss, per_slot_class_logits, per_slot_start_logits, per_slot_end_logits, per_slot_refer_logits,) + outputs[1:2]
