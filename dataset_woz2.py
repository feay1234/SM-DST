...
def create_examples(input_file, set_type, slot_list,
    ...
    for entry in input_data:
        ...
        for turn in entry['dialogue']:
            ...
            if set_type != "train" and perturbation > 0:
                ...
                # shuffle word
                    ...
                    for i in range(len(_hst)):
                        _hst[i] = " ".join(random.sample(_hst[i].split(), len(_hst[i].split())))
                # reverse word
                elif perturbation == 4:
                    for i in range(len(_hst)):
                ...
                hst_utt_tok = []
                for p in _hst:
                    hst_utt_tok.extend(p.split())

                # print(" ".join(hst_utt_tok))
            ...
            #     if diag_seen_slots_value_dict[sn] != "none":
            #         f = open("dataset_config/woz2.txt", "a")
            #         f.write("%s,%s\n" % (sn, diag_seen_slots_value_dict[sn]))
            #         f.close()
