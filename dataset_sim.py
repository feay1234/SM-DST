...
def create_examples(input_file, set_type, slot_list,
    ...
    if set_type == "train" and value_augment > 0:
        db_name = "sim-m.txt" if 'sim-M' in input_file else "sim-r.txt"
        slot2values = collections.defaultdict(set)
        tmp = open("dataset_config/%s" % db_name).read().splitlines()
        for i in tmp:
            k, v = i.split(",")
            slot2values[k].add(v)
    ...
    for entry in input_data:
        ...
        for turn_id, turn in enumerate(entry['turns']):
            ...
            # generate database
            # for sn in value_dict:
            #     if value_dict[sn] != "none":
            #         f = open("dataset_config/sim-m.txt", "a")
            #         f.write("%s,%s\n" % (sn, value_dict[sn]))
            #         f.close()
            ...
            if set_type == "train" and value_augment > 0:
                for augment_time in range(value_augment):
                    has_replace = False
                    new_txt_a, new_txt_b, new_diag_seen_slots_value_dict = txt_a.copy(), txt_b.copy(), value_dict.copy()

                    for sn in value_dict:
                        if value_dict[sn] not in ["dontcare", "none", "true", "false"]:

                            ori_value = tokenize(value_dict[sn])
                    ...
                    if has_replace:
                        # print(set(new_diag_seen_slots_value_dict.values()), set(value_dict.values()))
                        # print(txt_a)
                        # print(new_txt_a)
                        # print(txt_b)
                        # print(new_txt_b)
